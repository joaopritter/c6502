...
C6502 init_and_run_program(uint8_t program[]) {
  C6502 cpu = init_cpu();
  load(&cpu, program);
  run(&cpu);
  return cpu;
...
void test_0xa9(void) {
  ...
  C6502 cpu = init_and_run_program(program);
  TEST_ASSERT_EQUAL(exp, cpu.a);
}
...
void test_0x48(void) {
  ...
  C6502 cpu = init_and_run_program(program);
  TEST_ASSERT_EQUAL(exp, cpu.memory[0x01FF]);
}
...
void test_0x08(void) {
  ...
  C6502 cpu = init_and_run_program(program);
  TEST_ASSERT_EQUAL(0b00110100, cpu.memory[0x01FF]);
}
...
void test_0x68(void) {
  ...
  C6502 cpu = init_and_run_program(program);
  TEST_ASSERT_EQUAL(0b00110100, cpu.a);
}
